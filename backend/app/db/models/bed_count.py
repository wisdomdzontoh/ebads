"""``bed_count`` — live bed availability for non-simulation data sources (docs/02 §2.2).
...
``PATCH /facilities/{id}/beds``. Simulation runs do NOT use this table — they keep isolated
per-session state in ``simulation_bed_state`` (docs/02 §4 invariant).
"""
...
class BedCount(Base):
    """Available/total beds of one type at one facility (docs/02-data-model.md §2.2)."""
